...
miseq_path <- "/nfshome/store02/users/b.bsp81d/Lofresh_raw_data/WP2_gene_demultiplex/WP2_18S/"
...
fnFs <- sort(list.files(miseq_path,pattern="_cutout18S.1.fastq.gz"))
fnRs <- sort(list.files(miseq_path,pattern="_cutout18S.2.fastq.gz"))

sampleNames <- sapply(strsplit(basename(fnFs), "_cutout18S"), `[`, 1)
head(sampleNames)
...
filt_path <- "/nfshome/store02/users/b.bsp81d/Lofresh_raw_data/WP2_gene_demultiplex/WP2_18S/filtered"
...
write.table(track,"track_WP2_18S.txt")
...
write.table(taxTab,"taxTab_WP2_18S.txt")				 
write.table(seqtabNoC,"seqtabNoC_WP2_18S.txt")

samples.out <- rownames(seqtabNoC)
write.table(samples.out,"samples.out_WP2_18S.txt")
